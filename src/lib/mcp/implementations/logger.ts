...
export class MCPLogger {
	...
	static log(operation: string, input: any, result: MCPResult<any>): void {
		...
		// Performance metrics
		if (this.config.enablePerformanceTracking && result.metadata?.executionTime) {
			console.log(`⏱️ Execution time: ${result.metadata.executionTime.toFixed(2)}ms`);
		}
	 ...
	 */
	static performance(operation: string, startTime: number, endTime: number, metadata?: any): void {
		if (!this.config.enablePerformanceTracking) return;

		const executionTime = endTime - startTime;
		console.log(`📊 MCP Performance - ${operation}: ${executionTime.toFixed(2)}ms`, metadata);
	}
	 ...
	 */
	private static getLogLevel(result: MCPResult<any>): 'error' | 'warn' | 'info' | 'debug' {
