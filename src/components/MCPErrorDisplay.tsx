								...
								<div key={index} className="border border-red-600 rounded-lg">
									...
									>
										<div className="flex items-center justify-between">
											<div className="flex items-center gap-2">
												...
												<span className="text-red-400 font-medium">{error.code}</span>
												{error.field && (
											...
											<div className="flex items-center gap-2">
												...
												<i className={`pi ${isExpanded ? 'pi-chevron-up' : 'pi-chevron-down'} text-gray-400`}></i>
											</div>
										</div>
									...
									{isExpanded && showDetails && (
										<div className="px-3 pb-3 border-t border-gray-700">
											{error.details && (
												<div className="mt-3">
													<h6 className="text-sm font-medium text-gray-300 mb-2">Details:</h6>
													<div className="bg-gray-900 p-3 rounded text-sm">
														{error.details.expected && (
															<div className="mb-2">
																<span className="text-green-400">Expected:</span>
																<pre className="text-gray-300 mt-1">
																	{JSON.stringify(error.details.expected, null, 2)}
																</pre>
															</div>
														)}
														{error.details.actual && (
															<div className="mb-2">
																...
																<pre className="text-gray-300 mt-1">
																	{JSON.stringify(error.details.actual, null, 2)}
																</pre>
															</div>
														)}
