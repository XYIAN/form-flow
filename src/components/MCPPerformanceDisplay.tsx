...
export default function MCPPerformanceDisplay({ className = '' }: MCPPerformanceDisplayProps) {
		...
		if (isMonitoring) {
			...
			MCPLogger.configure({
				...
				onLog: (level, operation, message, data, executionTime) => {
					if (executionTime && level === 'info' && message.includes('completed')) {
	...
	const getOperationStats = () => {
		...
		return Object.entries(operationCounts).map(([operation, count]) => {
	...
	return (
		<Card className={`mcp-performance-display ${className}`}>
			...
			{metrics.length > 0 && (
				<div className="mb-4">
					<h5 className="text-sm font-medium text-white mb-2">Recent Performance</h5>
						...
						<div className="flex items-end gap-1 h-full">
							{metrics.slice(-20).map((metric, index) => (
								<div
									...
									style={{
										height: `${Math.min((metric.executionTime / Math.max(maxTime, 1)) * 100, 100)}%`,
										width: '8px',
										minHeight: '2px',
									}}
									title={`${metric.operation}: ${metric.executionTime.toFixed(2)}ms`}
								/>
							))}
						</div>
					</div>
			...
			{getOperationStats().length > 0 && (
				<div>
					<h5 className="text-sm font-medium text-white mb-2">Operation Statistics</h5>
					<div className="space-y-2">
						{getOperationStats().map((stat) => (
							<div key={stat.operation} className="p-3 bg-gray-800 rounded">
								<div className="flex justify-between items-center mb-2">
									<span className="text-white font-medium">{stat.operation}</span>
								...
								<div className="grid grid-cols-3 gap-2 text-xs">
									<div>
										...
										<span className={`ml-1 ${getPerformanceColor(stat.avgTime)}`}>
											{stat.avgTime.toFixed(2)}ms
										</span>
									</div>
									<div>
										<span className="text-gray-400">Max:</span>
										<span className={`ml-1 ${getPerformanceColor(stat.maxTime)}`}>
											{stat.maxTime.toFixed(2)}ms
										</span>
									</div>
									<div>
										<span className="text-gray-400">Min:</span>
										<span className={`ml-1 ${getPerformanceColor(stat.minTime)}`}>
