		...
		setHealthMetrics(prev => {
			...
			if (existing) {
				const updated = {
					...
					averageTime:
						(existing.averageTime * existing.totalCount + executionTime) /
	...
	useEffect(() => {
		...
		const avgTime =
			healthMetrics.length > 0
				? healthMetrics.reduce((sum, m) => sum + m.averageTime, 0) /
		...
		<Card className={`mcp-health-dashboard ${className}`}>
			<div className='flex justify-between items-center mb-4'>
				...
				<div className='flex items-center gap-2'>
					<i
						className={`pi ${getStatusIcon(systemStatus)} ${getStatusColor(
							systemStatus
						)}`}
					></i>
								...
								<div key={metric.operation} className='p-3 bg-gray-800 rounded'>
									<div className='flex justify-between items-center mb-2'>
										...
										<div className='flex items-center gap-2'>
											...
											<span
												className={`text-xs ${
													operationSuccessRate >= 95
														? 'text-green-400'
														: operationSuccessRate >= 80
														? 'text-yellow-400'
														: 'text-red-400'
												}`}
											>
