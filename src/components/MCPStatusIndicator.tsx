					...
					if (level === 'info' && message.includes('Starting')) {
						...
						setOperations(prev => prev.map(op => 
							op.operation === op.operation && op.status === 'running'
								? { ...op, status: 'success', endTime: Date.now(), executionTime: execTime }
								: op
						))
					} else if (level === 'error') {
						setOperations(prev => prev.map(op => 
							op.operation === op.operation && op.status === 'running'
								? { ...op, status: 'error', endTime: Date.now(), error: message }
								: op
						))
					}
				}
		...
		<Card className={`mcp-status-indicator ${className}`}>
			...
			<div className="mb-3">
				<div className="flex items-center gap-2 mb-2">
					...
					{status === 'running' && <ProgressSpinner style={{ width: '16px', height: '16px' }} />}
				</div>
			...
			<div className="grid grid-cols-3 gap-2 mb-3">
				...
				<div className="text-center p-2 bg-gray-800 rounded">
					<div className="text-blue-400 font-bold">{runningOperations.length}</div>
			...
			{isExpanded && (
				<div className="mt-3">
					<h5 className="text-sm font-medium text-white mb-2">Recent Operations</h5>
					<div className="space-y-2 max-h-40 overflow-y-auto">
						{recentOperations.length === 0 ? (
							...
							recentOperations.map((op) => (
								<div key={op.id} className="flex items-center justify-between p-2 bg-gray-800 rounded">
									<div className="flex items-center gap-2">
										<i className={`pi ${getStatusIcon(op.status)} text-sm ${
											op.status === 'success' ? 'text-green-400' :
											op.status === 'error' ? 'text-red-400' :
											op.status === 'running' ? 'text-blue-400' : 'text-gray-400'
										}`}></i>
										<span className="text-sm text-white">{op.operation}</span>
									</div>
									<div className="text-xs text-gray-400">
										{op.executionTime ? formatExecutionTime(op.executionTime) : 'Running...'}
									</div>
								</div>
							))
						)}
