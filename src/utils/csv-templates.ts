...
export const CSV_TEMPLATES: Record<string, CSVTemplate> = {
	contact: {
		...
		instructions:
			'This template includes common contact fields. Replace the sample data with your actual data, keeping the header row intact.',
	},
	...
	registration: {
		...
		instructions:
			'Perfect for event registrations. Include dietary restrictions, emergency contacts, and payment status tracking.',
	},
	...
	survey: {
		...
		instructions:
			'Ideal for customer feedback collection. Includes rating scales and permission tracking for follow-up contact.',
	},
	...
	application: {
		...
		instructions:
			'Complete job application template. Includes personal info, experience, education, and references.',
	},
	...
	feedback: {
		...
		sampleData: [
			...
			[
				...
				'Functionality',
				'Dark mode support',
				'Would like to see a dark mode option',
				'N/A',
		...
		instructions:
			'Comprehensive feedback collection template. Perfect for bug reports, feature requests, and user feedback.',
	},
}
...
export function downloadCSVTemplate(template: CSVTemplate): void {
	...
	if (link.download !== undefined) {
		...
		link.setAttribute(
			'download',
			`${template.name.replace(/\s+/g, '_')}_template.csv`
		)
...
export function getTemplatesByCategory(category: string): CSVTemplate[] {
	return Object.values(CSV_TEMPLATES).filter(
		template =>
